...
export default async function ArticlePage({ params }: Props) {
  ...
  const relatedArticles = filterArticles({
    ...
    return (
    <div className="max-w-7xl mx-auto px-4 py-10"> {/* Increased max-width for dual column */}
      <Link
        href="/"
      ...
      <div className="flex flex-col lg:flex-row gap-8"> {/* Main layout container */}

        {/* Main Article Section - Adjusted width for left alignment */}
        <div className="flex-grow lg:w-2/3"> {/* Takes 2/3 width on large screens, grows on small */}
            ...
            <CardHeader>
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <span>•</span>
                  <span>{moment(article.createdAt).format("DD MMM YYYY")}</span> {/* Corrected format for "2025" */}
        ...
        {relatedArticles.length > 0 && (
          <aside className="lg:w-1/3 flex-shrink-0"> {/* Takes 1/3 width on large screens, doesn't shrink on small */}
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4 lg:text-left text-center"> {/* Adjusted heading size and alignment */}
              More from {article.category.name}
            </h2>
            <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-1"> {/* Adjusted grid for sidebar */}
              {relatedArticles.map((relatedArticle) => (
                <Card
                  key={relatedArticle.id}
                  className="overflow-hidden shadow-md hover:shadow-lg transition-shadow bg-white dark:bg-gray-800"
                >
                  <Link href={`/articles/${relatedArticle.slug || relatedArticle.id}`}>
                    <Image
                      src={relatedArticle.imageUrl || "/placeholder-image.jpg"}
                    ...
                    <CardContent>
                      <p
                        ...
                        dangerouslySetInnerHTML={{
                          __html: relatedArticle.content.substring(0, 150) + (relatedArticle.content.length > 100 ? "..." : ""),
                        }}
                      />
                    </CardContent>
